...
// -----------------------------------------------------------------------------
uint64_t get_next_power2(const uint64_t value) {
    uint64_t power;
...
void bg_hash_constructor(bg_HashMap* map, size_t start_size) {
    ...
    map->_entries = calloc(MAP_START_SIZE, sizeof(bg_HashMapEntry));
}
...
void text_close_sink(bg_DataSink* data_sink) {
    ...
    bg_internal_verify(text_data_sink->_struct_type == BG_STRUCTTYPE_TEXTDATASINK);
...
// -----------------------------------------------------------------------------
void
bg_verify_fail(const char* expr, const char* file_name, uint32_t line_number,
               const char* function_name, const char* function_signature) {
    // TODO
}
...
{
  int ret = (int)syscall(__NR_perf_event_open, hw_event, pid, cpu,
                         group_fd, flags);
  return ret;
}
...
static int
open_event(uint32_t event_type, uint64_t event, int group_fd) {
  struct perf_event_attr perf_event_attr;
  uint64_t count;

  memset(&perf_event_attr, 0, sizeof(pe));
  perf_event_attr.type = event_type;
  perf_event_attr.size = sizeof(pe);
...
// -----------------------------------------------------------------------------
void bg_counter_handles_destructor(bg_CounterHandles* counter_handles) {
...
// -----------------------------------------------------------------------------
void bg_counters_destructor(bg_Counters* counters) {
...
void bg_program_constructor(bg_Program* bg_program_variable,
    ...
    bg_program_variable->_struct_type           = BG_STRUCTTYPE_PROGRAM;
